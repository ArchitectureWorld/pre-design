    ...
    <div className="pre-stage"><div className="pre-ambient" aria-hidden="true" />
      <div className="workspace">
        <header className="titlebar">
          <div className="brand"><span className="brand-mark" aria-hidden="true" /><div><strong>Pre-Design DSH</strong><small>AI for Design Workflow</small></div><span className="version">Pre {PRE_DESIGN_VERSION}</span></div>
          <div className="theme-switch" role="group" aria-label="液态玻璃主题">
            {GLASS_THEMES.map(theme => <button key={theme.id} type="button" aria-label={theme.label} title={theme.label} aria-pressed={appearance.theme === theme.id} onClick={() => setAppearance({ ...themeAppearance(theme.id), motion: appearance.motion })}><b>{theme.id.toUpperCase()}</b><span>{theme.label.slice(4)}</span></button>)}
          </div>
          <button ref={toggle} className="pre-button appearance-trigger" type="button" aria-label="外观" aria-expanded={open} aria-controls={`${unique}-appearance`} onClick={() => open ? dismiss() : setOpen(true)}><GlassIcon name="settings" /><span>外观</span></button>
        </header>
        ...
        {open && <section className="appearance-panel" role="region" aria-label="外观设置" id={`${unique}-appearance`} onKeyDown={event => { if (event.key === 'Escape') { event.stopPropagation(); dismiss() } }}>
          <header className="appearance-heading"><strong>液态玻璃 · 外观</strong><button className="pre-button" type="button" ref={close} aria-label="关闭外观设置" onClick={dismiss}><GlassIcon name="close" /></button></header>
          <p className="appearance-note">只调整本界面，不改变模型配置或正在执行的任务。C 与 A、D 与 B 使用相同面板参数。</p>
          ...
          <p className="appearance-note">折射为屏幕空间视觉模拟，不是物理折射率；不支持时保留磨砂与厚度效果，文字不参与扭曲。</p>
          ...
          <div className="appearance-footer"><button className="pre-button" type="button" aria-pressed={grid} onClick={() => setGrid(!grid)}>折射观察网格</button><button className="pre-button" type="button" onClick={() => setAppearance(themeAppearance(appearance.theme))}>还原外观</button></div>
          {!persisted && <p className="pre-warning" role="status">浏览器未允许保存外观，本次调整仍然有效。</p>}
        </section>}
      </div>
      <footer className="pre-bottom"><span>PRE-DESIGN / LIQUID GLASS</span><span>{GLASS_THEMES.find(theme => theme.id === appearance.theme)?.label} · 外观设置仅保存在本浏览器</span></footer>
    </div>
  </div>
}
