...
import { renderPlanningPage, PLANNING_PAGE_CSS, PLANNING_PRINT_CSS } from './render-planning-page.ts'
...
function renderPage(
  ...
  if (block?.type === 'planning-page') return `<section class="print-page" data-page-kind="${page.kind}"><div class="page-number">${index + 1} / ${context.plan.pages.length}</div>${renderPlanningPage(page.planningContent ?? block.page, block.chapterTitle, assets)}<footer>${escapeHtml(context.report.identity.projectName)} · 前期策划</footer></section>`
...
export async function renderPrintHtml(
  ...
  if (context.plan.canvas) return (await renderRegularHtml(context, outputRoot, true)).path
  ...
  const finalHtml = html
    .replace('</style>', `${PLANNING_PAGE_CSS}${PLANNING_PRINT_CSS}</style>`)
    ...
    .replace('</style>', isConditionalReport(context.report)
      ? '.print-page .page-copy h1{font-size:24pt}.print-page .focus{font-size:14pt;line-height:1.4;font-weight:400;white-space:pre-line}.print-page .focus.implementation-judgement{font-size:15pt;line-height:1.45;font-weight:400;white-space:pre-line}</style>'
      : '</style>')
