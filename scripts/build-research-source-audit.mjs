...
const specsDoc = await readJson('research/v2.0.1/workflow-research-specs.json')
...
const specByWorkflow = new Map(specsDoc.workflows.map(spec => [spec.workflowId, spec]))
...
for (const spec of specsDoc.workflows) {
...
function sourceCard(sourceId, purpose) {
  ...
  return `<div class="source">
    <div class="source-head"><strong>${esc(source.name)}</strong>${badge(source.priority)}${badge(source.reliabilityGrade)}${badge(status, cls)}</div>
    <div class="purpose">${esc(purpose ?? '')}</div>
    <div class="meta"><span>发布方：${esc(source.publisher)}</span><span>访问方式：${esc(source.accessModes.join(' / '))}</span></div>
    <div class="links">主入口：${link(source.homepage)}${audit?.workingUrl && audit.workingUrl !== source.homepage ? ` · 本次可工作入口：${link(audit.workingUrl)}` : ''}</div>
    <div class="note">${esc(audit?.note ?? source.notes)}</div>
  </div>`
}
...
function workflowHtml(workflow) {
  ...
  if (!spec) {
    return `<article class="workflow unmapped"><header><div><span class="workflow-id">${esc(workflow.work_item_id)}</span><strong>${esc(workflow.title)}</strong></div>${badge('ResearchSpec 待映射', 'warn')}</header><p>现有 V0.6 Workflow 保留执行；V2.0.1 尚未给此项建立“步骤 → 数据项 → 来源 → 分析 → 结论”的 Research 链，因此不得把它标记为来源可审计完成。</p></article>`
  }
  ...
  return `<article class="workflow mapped"><header><div><span class="workflow-id">${esc(workflow.work_item_id)}</span><strong>${esc(workflow.title)}</strong></div>${badge('Research 链已映射', 'ok')}</header>
    <div class="flow-summary"><span>Workflow：<code>${esc(spec.workflowId)}</code></span><span>最低证据：高权威 ${spec.minimumEvidence.minHighAuthority} / 独立来源 ${spec.minimumEvidence.minIndependentSources}</span></div>
    ${steps}
    <section class="methods"><h4>汇总与分析</h4><p><strong>汇总：</strong>${esc(spec.aggregationMethod.methodId)} v${esc(spec.aggregationMethod.version)} — ${esc(spec.aggregationMethod.description)}</p><p><strong>分析：</strong>${esc(spec.analysisMethod.methodId)} v${esc(spec.analysisMethod.version)} — ${esc(spec.analysisMethod.description)}</p><p><strong>缺失策略：</strong>${esc(spec.fallbackPolicy)}</p><h5>最终输出结论</h5><ul>${spec.outputClaims.map(claim => `<li>${esc(claim)}</li>`).join('')}</ul></section>
  </article>`
}
