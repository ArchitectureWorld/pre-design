...
describe('regular client physical pages', () => {
  it('retains every body character and shows each planned image once before wide continuations', () => {
    ...
    expect(parts.slice(1).every(p => p.layout.mode === 'text')).toBe(true)
  })
  it('supports all regular photo compositions and never discards a third or fourth image', () => {
    const modes = Array.from({ length: 5 }, (_, i) => planRegularManuscriptPage(page, '产品', assets(1), i)[0]!.layout.mode)
    expect(new Set(modes)).toEqual(new Set(['left', 'right', 'top', 'bottom', 'background']))
    for (const i of [0, 1]) {
      const parts = planRegularManuscriptPage(page, '产品', assets(4), i)
      expect(parts.flatMap(p => p.layout.media.map(a => a.assetId))).toEqual(['a0', 'a1', 'a2', 'a3'])
      expect(parts[0]!.layout.mode).toBe(i === 0 ? 'row' : 'column')
    }
  })
  it('gives a diagram its own large area, keeps prose and does not duplicate the diagram', () => {
    ...
    expect(parts[0]!.layout.media[0]!.box.w).toBeGreaterThan(11)
    expect(parts.map(p => p.content.body.join('')).join('')).toBe(page.body.join(''))
  })
  it('bounds every text/media rectangle and uses a 16:9 canvas', () => {
    const layouts = [regularCover('少潭河前期策划', '茶园与湖岸的日间休闲目的地', assets(1)[0]), ...planRegularManuscriptPage(page, '产品', assets(3), 0).map(p => p.layout)]
...
describe.each([
  ...
  it.each([
    { ordinal: 4, slot: 'background' }, { ordinal: 2, slot: 'top' }, { ordinal: 3, slot: 'bottom' },
    { ordinal: 0, slot: 'left' }, { ordinal: 1, slot: 'right' },
  ])('keeps the complete 960 × 1440 portrait at both vertical edges in the $slot rotation slot', ({ ordinal }) => {
    const asset = { ...assets(1)[0]!, width: 960, height: 1440 }
    const parts = planRegularManuscriptPage(input, '休憩场所', [asset], ordinal)
    ...
    // The displayed photograph, not merely its CSS rectangle, must reach both edges.
    const displayedHeight = asset.height * Math.min(image.box.w / asset.width, image.box.h / asset.height)
    expect(displayedHeight).toBeCloseTo(7.5, 6)
    expect(parts.flatMap(part => part.content.body).join('')).toBe(input.body.join(''))
  })

  it('also keeps a near-square portrait full-height without overlapping the copy', () => {
    const asset = { ...assets(1)[0]!, width: 900, height: 960 }
    for (const ordinal of [0, 1, 4]) {
      ...
      for (const item of [...layout.texts, image]) {
        expect(item.box.x).toBeGreaterThanOrEqual(0)
        expect(item.box.y).toBeGreaterThanOrEqual(0)
        expect(item.box.x + item.box.w).toBeLessThanOrEqual(13.333334)
        expect(item.box.y + item.box.h).toBeLessThanOrEqual(7.5)
        expect(item.box.w).toBeGreaterThan(0)
      }
      for (const text of layout.texts) {
        if (layout.mode === 'left') expect(text.box.x).toBeGreaterThanOrEqual(image.box.x + image.box.w)
        else expect(text.box.x + text.box.w).toBeLessThanOrEqual(image.box.x)
      }
    }
  })

  it('preserves the five landscape positions while keeping unreviewed subjects intact', () => {
    const modes = ['left', 'right', 'top', 'bottom', 'background']
    for (const [ordinal, mode] of modes.entries()) {
      const layout = planRegularManuscriptPage(input, '休憩场所', assets(1), ordinal)[0]!.layout
  ...
  ])('safely contains uncertain dimensions in the existing background position for $label', ({ dimensions }) => {
    const asset = { ...assets(1)[0]!, ...dimensions } as ClientVisualAsset
    const layout = planRegularManuscriptPage(input, '休憩场所', [asset], 4)[0]!.layout
    expect(layout.mode).toBe('background')
    expect(layout.media[0]).toMatchObject({ fit: 'contain', box: { x: 0, y: 0, w: 13.333333, h: 7.5 } })
    expect(layout.shade).toEqual({ x: 0, y: 0, w: 8.85, h: 7.5 })
  })

  it.each(['diagram', 'map', 'chart'] as const)('preserves the existing large contained %s composition', role => {
    ...
    expect(layout.media[0]!.fit).toBe('contain')
    expect(layout.media[0]!.box.w).toBeGreaterThan(11)
    expect(layout.shade).toBeUndefined()
  })
})
