...
- [ ] 现有 949 项合同测试全部通过，另补充 DSH Adapter 的集成测试与端到端测试。
...
`T01—T47` 为 Workflow 内部能力，不直接暴露给模型。模型输出必须先形成结构化命令，经 `preplanning_apply_commands` 进入 ProposalEnvelope、Schema/Rule/Permission/Revision 校验，再由系统服务或人工完成状态提交。Gate 只能由授权自然人批准。

## DSH 原生交付身份

```text
repository: dsh-preplanning-agent
package: @architectureworld/dsh-preplanning-agent
plugin_id: preplanning-agent
```

第一实现形态为单 Bundle（Host + Browser），不先做独立 Web、通用 Runtime Adapter 产品或产品级 Mock Runtime。
